...
def analyze(symbol, mbonus, market_regime):
    ...
    score = 0
    detail = []  # rincian skor: (nama, poin, nilai)
    ...
    elif rv>75: score-=4; detail.append((f"RSI {rv:.0f} (overbought >75, rawan koreksi)", -4))
    ...
    score += accum_bonus
    if accum_bonus: detail.append(("🐋 Akumulasi terdeteksi (bandar masuk)", accum_bonus))
    is_breakout, bo_bonus = detect_breakout(cl, hi, vol)
    score += bo_bonus
    if bo_bonus: detail.append(("🚀 Breakout terkonfirmasi (tembus resistance)", bo_bonus))

    if market_regime == "🔴 BEARISH":
        if harga > e20[-1] and wtrend == "BULLISH":
            score += 3; detail.append(("Kuat saat market bearish (hidden strength)", +3))
        else:
            score += mbonus; detail.append((f"Penalti market bearish", mbonus))
    else:
        score += mbonus
        if mbonus: detail.append((f"Bonus market regime", mbonus))
    ...
    of = 0
    if nearhigh: of+=3; detail.append(("Candle ditutup dekat high (kuat)", +3))
...
with c1:
    ...
    st.caption("Versi dengan Rincian Skor Teknikal")
...
with t5:
    ...
    st.caption("Lihat rincian skor per indikator untuk memahami KENAPA sebuah saham dapat sinyal tertentu.")
    if results:
        ...
        detail = s.get("detail",[])
        if detail:
            # Tampilkan sebagai tabel berwarna
            rows = []
            ...
            st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True, height=min(420, 40*(len(rows)+1)))
            ...
            st.markdown("""
            - 🟢 **Poin plus** = faktor mendukung kenaikan (tren naik, volume kuat, momentum positif)
            - 🔴 **Poin minus** = faktor risiko (overbought, distribusi, fake breakout)
            - ⚪ **Nol** = netral (tidak menambah/mengurangi)
            
            Makin tinggi total skor + R/R bagus + Weekly BULLISH = sinyal makin kuat.
            """)
        else:
