...
# Database lengkap saham per sektor (LQ45 + small/mid cap + populer)
SEKTOR_LENGKAP = {
    "🏦 BANK & FINANSIAL": [
        "BBCA.JK","BBRI.JK","BMRI.JK","BBNI.JK","BBTN.JK","BRIS.JK","BNGA.JK",
        "NISP.JK","BJBR.JK","BJTM.JK","BTPS.JK","AMAR.JK","BBKP.JK","BFIN.JK",
        "PNLF.JK","ADMF.JK","BNII.JK","MEGA.JK","BNLI.JK","BBYB.JK"
    ],
    "📡 TELEKOMUNIKASI & MEDIA": [
        "TLKM.JK","ISAT.JK","EXCL.JK","FREN.JK","TOWR.JK","TBIG.JK","MTEL.JK",
        "LINK.JK","SCMA.JK","MNCN.JK","SUPR.JK"
    ],
    "⛏️ TAMBANG & LOGAM": [
        "ADRO.JK","ITMG.JK","PTBA.JK","ANTM.JK","INCO.JK","MDKA.JK","AMMN.JK",
        "HRUM.JK","DOID.JK","BRMS.JK","NCKL.JK","ABMM.JK","BYAN.JK","BUMI.JK",
        "INDY.JK","KKGI.JK","TINS.JK","PSAB.JK","ADMR.JK"
    ],
    "⚡ ENERGI & UTILITAS": [
        "PGAS.JK","MEDC.JK","ELSA.JK","DEWA.JK","RAJA.JK","ENRG.JK","WIKA.JK",
        "PTPP.JK","PJAA.JK","WSKT.JK","BIPI.JK","ESSA.JK"
    ],
    "🍜 KONSUMER & MAKANAN": [
        "ICBP.JK","INDF.JK","UNVR.JK","MYOR.JK","GGRM.JK","HMSP.JK","SIDO.JK",
        "KLBF.JK","CPIN.JK","JPFA.JK","MAIN.JK","CMRY.JK","ROTI.JK","ULTJ.JK",
        "CLEO.JK","KEJU.JK","STTP.JK","DLTA.JK","MLBI.JK"
    ],
    "🏭 INDUSTRI & MANUFAKTUR": [
        "ASII.JK","UNTR.JK","AKRA.JK","SMGR.JK","INTP.JK","SMCB.JK","ARNA.JK",
        "TOTO.JK","SRTG.JK","KRAS.JK","ISSP.JK","JECC.JK","IPOL.JK"
    ],
    "🏢 PROPERTI & KONSTRUKSI": [
        "CTRA.JK","SMRA.JK","PWON.JK","BSDE.JK","ASRI.JK","DMAS.JK","KIJA.JK",
        "PANI.JK","LPCK.JK","ADHI.JK","JKON.JK","NRCA.JK","TOTL.JK","WSBP.JK"
    ],
    "🏥 KESEHATAN & FARMASI": [
        "KLBF.JK","SIDO.JK","MIKA.JK","HEAL.JK","PRDA.JK","SILO.JK","KAEF.JK",
        "INAF.JK","TSPC.JK","SOHO.JK","PEHA.JK","DVLA.JK"
    ],
    "🛒 RITEL & PERDAGANGAN": [
        "AMRT.JK","MAPI.JK","ACES.JK","MAPA.JK","MAPB.JK","ERAA.JK","RALS.JK",
        "LPPF.JK","HERO.JK","MIDI.JK","CSAP.JK","ACST.JK"
    ],
    "💻 TEKNOLOGI & AI": [
        "GOTO.JK","BUKA.JK","EMTK.JK","DCII.JK","EDGE.JK","MLPT.JK","MTDL.JK",
        "WIFI.JK","DMMX.JK","MSIN.JK","CYBR.JK","WIRG.JK","DIVA.JK","KIOS.JK"
    ],
    "🚢 TRANSPORTASI & LOGISTIK": [
        "GIAA.JK","SMDR.JK","MIRA.JK","CMPP.JK","WEHA.JK","BIRD.JK","TMAS.JK",
        "ASSA.JK","TPMA.JK","SDMU.JK"
    ],
    "🌱 AGRIKULTUR & PERKEBUNAN": [
        "AALI.JK","LSIP.JK","SIMP.JK","SGRO.JK","SSMS.JK","TBLA.JK","BWPT.JK",
        "DSNG.JK","ANJT.JK","PALM.JK"
    ],
    "🏗️ INFRASTRUKTUR & SEMEN": [
        "JSMR.JK","META.JK","CASS.JK","SMGR.JK","INTP.JK","SMCB.JK","CMNP.JK",
        "WTON.JK","WSBP.JK","WIKA.JK"
    ],
}

# Default sektor pertama yang dipilih
if 'sektor_pilih' not in st.session_state:
    st.session_state.sektor_pilih = list(SEKTOR_LENGKAP.keys())[0]

# WATCHLIST aktif = sektor yang dipilih (hapus duplikat)
WATCHLIST = list(dict.fromkeys(SEKTOR_LENGKAP[st.session_state.sektor_pilih]))

def get_sector_short(symbol):
    """Cari sektor pendek (untuk display di card)."""
    code = symbol.upper()
    for sek, lst in SEKTOR_LENGKAP.items():
        if code in lst: return sek.split(" ",1)[1] if " " in sek else sek
    return "LAINNYA"
...
def analyze(symbol, mbonus, market_regime):
    ...
    sector = get_sector_short(symbol)
...
def backtest_symbol(symbol, target_pct=5, sl_pct=3, hold_days=6):
    ...
    for i in range(60, len(cl)-hold_days):
        ...
        # Sinyal DIPERKETAT: tren + RSI + MACD + volume + smart money + R/R
        sig = (sub[-1]>e20[-1]>e50[-1] and 45<=rv<=68 and ml>ms and vr>=1.3
               and smart and rr>=1.2)
        if sig:
            entry = cl[i]
...
# ═══════════════ HEADER: PILIH SEKTOR DI ATAS ═══════════════
sektor_list = list(SEKTOR_LENGKAP.keys())

c1,c2 = st.columns([3,1])
with c1:
    st.markdown(f"# 🎯 Scanner Sektor")
    st.caption(f"Pilih sektor di bawah · Swing 1-6 hari · Target 5%+ · {datetime.now().strftime('%d %b %Y · %H:%M')}")
with c2:
    if st.button("🔄 Refresh", use_container_width=True, type="primary"):
        st.cache_data.clear(); st.session_state.results = None; st.rerun()

# Dropdown sektor besar di atas (mudah diakses di HP)
st.markdown("### 📂 Pilih Sektor:")
pilihan = st.selectbox(
    "Sektor yang ingin dipindai:",
    sektor_list,
    index=sektor_list.index(st.session_state.sektor_pilih),
    key="sektor_selector",
    label_visibility="collapsed"
)
if pilihan != st.session_state.sektor_pilih:
    st.session_state.sektor_pilih = pilihan
    st.session_state.results = None  # paksa scan ulang
    st.cache_data.clear()
    st.rerun()

# Tampilkan info sektor yang aktif
n_saham = len(SEKTOR_LENGKAP[pilihan])
st.success(f"**{pilihan}** · 📊 {n_saham} saham siap dipindai")

# Daftar saham di sektor (bisa dibuka)
with st.expander("📋 Lihat semua saham di sektor ini"):
    saham_list = [s.replace(".JK","") for s in SEKTOR_LENGKAP[pilihan]]
    st.markdown(" · ".join([f"**{s}**" for s in saham_list]))
...
if st.session_state.results is None:
    ...
    if not res:
        st.warning("Tidak ada data saham yang berhasil diambil. Coba Refresh atau pilih sektor lain.")
