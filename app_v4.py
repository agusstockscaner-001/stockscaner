...
def backtest_symbol(symbol, target_pct=5, sl_pct=3, hold_days=6):
    ...
    wins=losses=total=0
    # Step 2 hari (bukan tiap hari) supaya backtest ringan, hindari timeout
    for i in range(60, len(cl)-hold_days, 2):
        sub = cl[:i]
        subhi = hi[:i]
        ...
        # FITUR BARU: Entry grade filter (anti-FOMO) — tolak kalau harga > 5% di atas EMA20
        dist_ema20 = (cl[i]-e20[-1])/e20[-1]*100
        not_fomo = dist_ema20 <= 5

        # Sinyal DIPERKETAT + anti-FOMO
        sig = (sub[-1]>e20[-1]>e50[-1] and 45<=rv<=68 and ml>ms and vr>=1.3
               and smart and rr>=1.2 and not_fomo)
        if sig:
            entry = cl[i]
